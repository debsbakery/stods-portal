...
export default function CostSettingsView({
  ...
  const profitTarget = 100 - labourNum - 30
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <div className="flex items-start justify-between">
            <div>
              <label className="block text-sm font-semibold text-gray-700">Global Labour %</label>
            ...
            <div className="text-right">
              ...
              <div className="mt-1">
                {labourActual ? (
                  <span className="text-xs bg-green-100 text-green-700 font-medium px-2 py-0.5 rounded-full">Actual</span>
                ) : (
                  <span className="text-xs bg-amber-100 text-amber-700 font-medium px-2 py-0.5 rounded-full">Estimate</span>
                )}
              </div>
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox" checked={labourActual}
              onChange={(e) => setLabourActual(e.target.checked)}
              className="w-4 h-4 rounded border-gray-300 text-indigo-600"
            />
            <span className="text-sm text-gray-600">Mark as <strong>actual</strong> (based on real time tracking)</span>
          </label>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Notes</label>
            <input
              type="text" value={labourNote}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <div className="flex items-start justify-between">
            <div>
              <label className="block text-sm font-semibold text-gray-700">Overhead Rate ($ per kg)</label>
              <p className="text-xs text-gray-400 mt-0.5">
                Product weight (kg) x this rate. Covers rent, power, packaging, equipment.
              </p>
            </div>
            <div className="text-right">
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox" checked={overheadActual}
              onChange={(e) => setOverheadActual(e.target.checked)}
              className="w-4 h-4 rounded border-gray-300 text-indigo-600"
            />
            <span className="text-sm text-gray-600">Mark as <strong>actual</strong> (based on real expense tracking)</span>
          </label>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Notes</label>
            <input
              type="text" value={overheadNote}
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
          <p className="text-sm font-semibold text-gray-700 mb-4">
            ...
            <span className="text-xs font-normal text-gray-400 ml-2">(overhead varies by product weight)</span>
          </p>
          <div className="space-y-3">
            {([
              { label: 'Ingredients (estimated avg)', pct: 30, color: 'bg-amber-400', note: null },
              { label: 'Labour', pct: labourNum, color: 'bg-blue-400', note: null },
              { label: 'Overhead', pct: null, color: 'bg-purple-400', note: '$' + overheadNum.toFixed(2) + '/kg' },
              { label: 'Profit target (estimated)', pct: profitTarget, color: profitTarget >= 10 ? 'bg-green-400' : 'bg-red-400', note: null },
            ] as { label: string; pct: number | null; color: string; note: string | null }[]).map((row) => (
              <div key={row.label} className="flex items-center gap-3">
                ...
                <span className="text-sm font-semibold text-gray-800 tabular-nums">
                  {row.note ?? (row.pct != null ? row.pct.toFixed(1) + '%' : '')}
                </span>
              </div>
            ))}
          </div>
          <div className="mt-4 flex rounded-full overflow-hidden h-4 gap-px">
            ...
            <div className="bg-blue-400" style={{ width: Math.min(labourNum, 100) + '%' }} />
            <div className="bg-purple-400" style={{ width: '10%' }} />
            <div className={profitTarget >= 10 ? 'bg-green-400' : 'bg-red-400'} style={{ width: Math.max(0, Math.min(profitTarget, 100)) + '%' }} />
          </div>
          {profitTarget < 10 && (
            <p className="text-xs text-red-600 mt-3 font-medium">Warning: estimated profit margin is below the 10% target.</p>
          )}
        </div>
