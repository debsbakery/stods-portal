...
export async function GET(request: NextRequest) {
  try {
    ...
    const ledger = allTransactions.map((tx: any) => {
      const isDebit = !tx.isPayment && ['invoice', 'charge', 'late_fee'].includes(tx.type)
      ...
      return {
        ...
        outstanding: tx.isPayment ? 0 : tx.amount - tx.amount_paid,
    ...
    return NextResponse.json({
      ...
      ledger,
      summary: {
        total_charges:   ledger.reduce((s: number, t: any) => s + t.debit,  0),
        total_payments:  ledger.reduce((s: number, t: any) => s + t.credit, 0),
        current_balance: runningBalance,
      },
    })
    ...
    return NextResponse.json({ error: error.message || 'Failed to generate ledger' }, { status: 500 })
  }
}
