...
// ── Skip weekly-billing customers from daily batch ────────────────────────
// Their invoices are generated by the Sunday weekly cron instead.
// If a specific customer_id is passed, honour it regardless of billing type.
const { data: weeklyCustomers } = await supabase
  .from('customers')
  .select('id')
  .eq('invoice_frequency', 'weekly')

const weeklyCustomerIds = (weeklyCustomers ?? []).map((c: any) => c.id)
...
if (customer_id) {
  // Explicit customer_id always honoured (admin manually invoicing one customer)
  ordersQuery = ordersQuery.eq('customer_id', customer_id)
} else if (weeklyCustomerIds.length > 0) {
  // Batch run: exclude weekly-billing customers
  ordersQuery = ordersQuery.not('customer_id', 'in', `(${weeklyCustomerIds.join(',')})`)
}
