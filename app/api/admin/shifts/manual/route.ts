...
export async function POST(request: NextRequest) {
  ...
  if (force_out && shift_id && clock_out_time) {
    ...
    try {
      calc = calculateShift({
        ...
        employmentType:           openShift.employment_type ?? openShift.staff?.employment_type,
        dayType,
        baseHourlyRate:           openShift.applicable_rate ?? openShift.staff?.base_hourly_rate,
        saturdayRate:             openShift.staff?.saturday_rate,
        sundayRate:               openShift.staff?.sunday_rate,
        publicHolidayRate:        openShift.staff?.public_holiday_rate,
        publicHolidayMultiplier:  openShift.staff?.public_holiday_multiplier,
        overtimeThresholdHours:   openShift.staff?.overtime_threshold_hours,
        overtimeMultiplier:       openShift.staff?.overtime_multiplier,
        doubleTimeThresholdHours: openShift.staff?.double_time_threshold_hours,
        doubleTimeMultiplier:     openShift.staff?.double_time_multiplier,
        salaryWeekly:             openShift.staff?.salary_weekly,
        salaryHoursPerWeek:       openShift.staff?.salary_hours_per_week,
        superRatePercent:         openShift.staff?.super_rate_percent,
        trueHourlyCost:           openShift.staff?.true_hourly_cost,
      })
    } catch (e) {
      console.error('[force clock-out] calc error:', e)
    }
    ...
    const { error: updateErr } = await supabase
      ...
      .update({
        ...
        paid_minutes:         calc?.paidMinutes         ?? paidMins,
        paid_hours:           calc?.paidHours           ?? paidHours,
        applicable_rate:      calc?.applicableRate      ?? openShift.applicable_rate,
  ...
  const staffBreakMins = Number(staff.break_minutes ?? 0)
  const section = await resolveSection()
  const breakMins = (section === 1 && grossMins >= 300) ? staffBreakMins : 0
