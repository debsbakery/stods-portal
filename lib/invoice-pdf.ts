...
export async function generateInvoicePDF(data: InvoiceData): Promise<jsPDF> {
  ...
  if (logoBase64) {
    try {
      doc.addImage(logoBase64, 'PNG', margin, yPos + 4, 90, 40)
    } catch {
  ...
  const tableData = order.order_items.map(item => {
    ...
    const displayName = (item as any).custom_description || item.product_name
    const lineTotal   = item.subtotal ?? (item.unit_price * item.quantity)
    return [
  ...
  const gstTotal = order.order_items.reduce((sum, item) => {
    const hasGST    = item.gst_applicable !== false
    const lineTotal = item.subtotal ?? (item.unit_price * item.quantity)
    return sum + (hasGST ? lineTotal * 0.1 : 0)
  }, 0)
